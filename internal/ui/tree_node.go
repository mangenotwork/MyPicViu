...
func buildTreeData() []*TreeNode {
	...
	err := db.DB.Get(db.TreeNodeTable, db.TreeNodeKey, &data)
	if err != nil {
...
func (m *TreeDataManager) AddRootFileNode(name, filePath string) bool {
	...
	err := db.DB.Set(db.TreeNodeTable, db.TreeNodeKey, &TreeData)
	if err != nil {
...
func CreateCustomTree() (*widget.Tree, *TreeDataManager) {
	...
	tree = widget.NewTree(
		...
		func(branch bool) fyne.CanvasObject {
			if branch {
				return NewTreeClickableLabel("目录", "")
			}
			return NewTreeClickableLabel("图片", "")
		},
		...
		func(id widget.TreeNodeID, branch bool, o fyne.CanvasObject) {
			...
			if label, ok := o.(*TreeClickableLabel); ok {
...
// 自定义可点击标签
type TreeClickableLabel struct {
	*widget.Label
	NodeID string
}

func (c *TreeClickableLabel) Tapped(event *fyne.PointEvent) {
...
func (c *TreeClickableLabel) TappedSecondary(event *fyne.PointEvent) {
	logger.DebugF("右键点击了节点 [ID: %s, 文本: %s, 位置: %v]",
		c.NodeID, c.Text, event.AbsolutePosition, event.Position)
	menuPos := fyne.NewPos(event.AbsolutePosition.X, event.AbsolutePosition.Y)
	popup := widget.NewPopUpMenu(popupMenu, MainWindow.Canvas())
	popup.Resize(fyne.NewSize(160, 260))
	popup.ShowAtPosition(menuPos)
}

// 初始化时必须调用
func NewTreeClickableLabel(text, nodeID string) *TreeClickableLabel {
	l := &TreeClickableLabel{
